...
/* Edited @Matthew Walker 01/01/21 - broadband df add threshold value
                                   - rename class
...
detection_function::detection_function( DFConfig Config ) :
    m_window(0)
{
    m_magHistory = NULL;
    m_phaseHistory = NULL;
...
void detection_function::initialise( DFConfig Config )
{
...
void detection_function::deInitialise()
{
    delete [] m_magHistory ;
...
double detection_function::processTimeDomain(const double *samples)
{
...
double detection_function::processFrequencyDomain(const double *reals,
                                                 const double *imags)
{
...
void detection_function::whiten()
{
...
double detection_function::runDF()
{
    double retVal = 0;
    // std:raise(SIGUSR1);
    switch( m_DFType )
    ...
    case DF_BROADBAND:
        ...
        break;
    default:
        std::cout << "Invalid dftype" <<std::endl;
    }
	
    return retVal;
}

double detection_function::HFC(unsigned int length, double *src)
{
    unsigned int i;
    double val = 0;
...
double detection_function::specDiff(unsigned int length, double *src)
{
    unsigned int i;
    double val = 0.0;
    double temp = 0.0;
...
double detection_function::phaseDev(unsigned int length, double *srcPhase)
{
    unsigned int i;
    double tmpPhase = 0;
    double tmpVal = 0;
...
double detection_function::complexSD(unsigned int length, double *srcMagnitude, double *srcPhase)
{
    unsigned int i;
    double val = 0;
    double tmpPhase = 0;
...
double detection_function::broadband(unsigned int length, double *src)
{
    double val = 0;
    if (src[0] < 0.001) {    // get rid of erroneous onset at start of track (I hope)
        return val;
    }
...
double* detection_function::getSpectrumMagnitude()
{
    return m_magnitude;
}
