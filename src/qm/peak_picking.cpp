...
peak_picking::peak_picking( PPickParams Config )
{
    m_workBuffer = NULL;
    initialise( Config );
}
...
void peak_picking::initialise( PPickParams Config )
{
...
void peak_picking::deInitialise()
{
    delete [] m_workBuffer;
...
void peak_picking::process( double* src, unsigned int len, vector<int> &onsets )
{
    if (len < 4) return;
...
int peak_picking::quadEval( vector<double> &src, vector<int> &idx )
{
