...
/* Edited @Matthew Walker 01/01/21 - m_div added and algorithm updated to scale with m_div to increase resolution
*/
...
tempo_track::tempo_track(float rate, size_t increment, int step_div) :
    m_rate(rate), m_increment(increment), m_div(step_div) { }
tempo_track::~tempo_track() { }

void
tempo_track::filter_df(d_vec_t &df)
{
    d_vec_t a(3);
    d_vec_t b(3);
...
// as it was before
void
tempo_track::calculateBeatPeriod(const vector<double> &df,
                                  ...
                                  double inputtempo, bool constraintempo)
{
    ...
    // and get best path
    // std::cout << "Using mdiv" << std::to_string(m_div) << std::endl;
    unsigned int wv_len = 128*m_div;
    ...
    double rayparam = (60*44100/(512/m_div))/inputtempo; // 64 for step_div=4, 8 for 8
...
//    std::cerr << "constraintempo" << constraintempo << std::endl;
    ...
    unsigned int winlen = 512*m_div;
    unsigned int step = 128*m_div;
...
void
tempo_track::get_rcf(const d_vec_t &dfframe_in, const d_vec_t &wv, d_vec_t &rcf)
{
...
void
tempo_track::viterbi_decode(const d_mat_t &rcfmat, const d_vec_t &wv, d_vec_t &beat_period, d_vec_t &tempi)
{
...
double
tempo_track::get_max_val(const d_vec_t &df)
{
    double maxval = 0.;
    for (unsigned int i=0; i<df.size(); i++)
    {
...
int
tempo_track::get_max_ind(const d_vec_t &df)
{
    double maxval = 0.;
    int ind = 0;
    for (unsigned int i=0; i<df.size(); i++)
...
void
tempo_track::normalise_vec(d_vec_t &df)
{
    double sum = 0.;
    for (unsigned int i=0; i<df.size(); i++)
    {
...
void
tempo_track::calculateBeats(const vector<double> &df,
