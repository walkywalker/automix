...
// All timing is set relative to original tempo and start time, then shifted in map_actions
...
tune::tune(std::string track_path, double tempo, double track_start_time, std::vector<std::pair<int, int>> drops, std::vector<int> four_bar_drum_content, bool analysis_success):
	...
	m_analysis_success(analysis_success) {
...
tune::tune(pugi::xml_node tune_node) {
		...
		if (m_analysis_success) {
			...
			for (pugi::xml_attribute_iterator ait = tune_node.child("drums").attributes_begin(); ait != tune_node.child("drums").attributes_end(); ++ait) {
...
int tune::get_drums(int bar_idx) {
	return m_drums[bar_idx/4];
}

double tune::get_original_start_time() {
	return m_track_start_time;
}
...
void tune::set_volume_ramp(double start_time, double end_time, double start_volume, double end_volume, int num_steps) {
