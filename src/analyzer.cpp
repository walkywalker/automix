...
std::shared_ptr<tune> analyzer::construct_tune(const double tempo,
                                               const double first_beat) {
  m_analysis_log_file << "number of bass steps: "
                      << std::to_string(m_bass_content.size()) << std::endl;
  std::for_each(m_bass_content.begin(), m_bass_content.end(),
                [this](double &n) {
                  m_analysis_log_file << std::to_string(n) << std::endl;
                });
  ...
  for (int step_idx = 0; step_idx < m_bass_content.size(); step_idx++) {
    if ((step_idx * step_base) > (first_beat * 44100)) {
      if ((step_idx * step_base) < section_count) {
        ...
        m_analysis_log_file << "step_idx " << std::to_string(step_idx) << " "
                            << std::to_string(double(section_count) / 44100)
                            << std::endl;
  ...
  while (time_temp < m_onset_features.back()) {
    four_bar_drum_content.push_back(
        get_onsets_in_range(time_temp, time_temp + four_bar_time));
    time_temp += four_bar_time;
  }

  std::for_each(four_bar_bass_content.begin(), four_bar_bass_content.end(),
                [this](double &n) {
                  m_analysis_log_file << std::to_string(n) << std::endl;
                });
  std::for_each(four_bar_drum_content.begin(), four_bar_drum_content.end(),
                [this](int &n) {
                  m_analysis_log_file << std::to_string(n) << std::endl;
                });

  auto bass_minmax = std::minmax_element(four_bar_bass_content.begin(),
                                         four_bar_bass_content.end());
  double bass_range = (*bass_minmax.second - *bass_minmax.first);

  m_analysis_log_file << "bass range " << std::to_string(bass_range)
                      << std::endl;
  ...
  for (auto bass_val : four_bar_bass_content) {
    if ((*bass_minmax.second - bass_val) < (0.4 * bass_range)) {
      m_analysis_log_file << "within range at bar  " << std::to_string(bar)
                          << std::endl;
      if (in_drop == 0) {
        start = bar;
        in_drop = 1;
      ...
      if (in_drop == 1) {
        ...
        m_analysis_log_file << "drop start: " << std::to_string(start)
                            << " drop end: " << std::to_string(bar)
                            << std::endl;
        in_drop = 0;
      }
    }
  ...
  if (in_drop == 1) {
    drops.push_back(std::make_pair(start, bar));
    m_analysis_log_file << "drop start: " << std::to_string(start)
                        << " drop end: " << std::to_string(bar) << std::endl;
  }
  ...
  return std::make_shared<tune>(m_track.get_path(), tempo, first_beat, m_vol,
                                drops, four_bar_drum_content, true);
}
...
void analyzer::run() {
  ...
  int min_step_size = std::min_element(m_processes.begin(), m_processes.end(),
                                       [](detector_helper const &s1,
                                          detector_helper const &s2) {
  ...
  while (m_track.read(interleaved_samples, min_step_size * 2) ==
         min_step_size * 2) {
    for (int i = 0; i < (min_step_size * 2) - 1; i = i + 2) {
      mono_samples[(max_window_size - min_step_size) + (i / 2)] =
          (interleaved_samples[i] + interleaved_samples[i + 1]) * 0.5;
      if ((!noise_found) &&
          ((interleaved_samples[i] + interleaved_samples[i + 1]) * 0.5) >
              m_noise_threshold) {
        m_first_noise = (buf_level + (i / 2)) / 44100;
        noise_found = true;
        m_analysis_log_file << "First noise found at time "
                            << std::to_string(m_first_noise)
                            << std::endl; // should be separate detectpr
      }
    }
    ...
    for (detector_helper helper : m_processes) {
      if (buf_level % helper.step_size == 0 && buf_level >= max_window_size) {
        helper.process(&mono_samples, Vamp::RealTime::frame2RealTime(
                                          buf_level - helper.window_size,
                                          44100)); // time at start of window
      }
    }
...
int analyzer::process() {
  ...
  m_analysis_log_file << "Using detection function "
                      << beat_analyzer.getParameter(df) << std::endl;
  ...
  if (detector.initialise(1, step_base, window_size) != true) {
    m_analysis_log_file << "Error initialising onset detector plugin"
                        << std::endl;
    return 1;
  }
  ...
  m_analysis_log_file << "num onsets: "
                      << std::to_string(m_onset_features.size()) << std::endl;
  if (m_onset_features.size() > 60) {
    for (int i = 0; i < m_onset_features.size(); i++) {
      m_analysis_log_file << "time: " << m_onset_features[i] << std::endl;
      m_analysis_log_file << "feature: " << o_features[i].values[0]
                          << std::endl;
    }
  }
...
int analyzer::get_bpm(double &bpm,
                      double &time) { // time will be a plugin beat somewhere
  std::vector<double> beat_gaps;
  double mean;
  double sd;
  for (int i = 0; i < m_beat_features.size() - 1; i++) {
    if (m_beat_features[i] >
        m_onset_features[0]) { // only consider beats past first drum
  ...
  m_analysis_log_file << "mean within stddev"
                      << std::to_string(double(60) / (cntr / total))
                      << std::endl;

  bpm = round((double(60) / (cntr / total)) * 4) /
        4; // round to nearest 0.25 bpm remove?
  ...
  for (beat_idx = 1; beat_idx < m_beat_features.size() - 1; beat_idx++) {
    if (m_beat_features[beat_idx] >
        m_onset_features[0]) { // could be more rigourous about ensuring there
                               // are drums in the section we chose here
      if ((std::abs(m_beat_features[beat_idx] - m_beat_features[beat_idx - 1] -
                    mean) < (2 * sd)) &&
          (std::abs(m_beat_features[beat_idx + 1] - m_beat_features[beat_idx] -
                    mean) < (2 * sd))) {
        num_beats++;
      } else {
  ...
  if (found_section == 0 && num_beats < consecutive_beat_threshold) {
    m_analysis_log_file
        << "ERROR could not find consistent beat pattern section" << std::endl;
    return 1;
  } else {
    m_analysis_log_file << "Found " << std::to_string(num_beats)
                        << " candidate beats beginning at beat "
                        << std::to_string(beat_idx - num_beats) << std::endl;
  }
  ...
  std::for_each(consistent_beats.begin(), consistent_beats.end(),
                [this](double &n) {
                  m_analysis_log_file << std::to_string(n) << std::endl;
                });
  std::for_each(beat_template.begin(), beat_template.end(), [this](double &n) {
    m_analysis_log_file << std::to_string(n) << std::endl;
  });
...
int analyzer::align_beat_grid(
    const double &bpm,
    double &time) { // time will be our best guess at first beat
  int resolution = 8;
  ...
  if (silence_to_first_onset < -1 * (1024.0 / 44100)) {
    m_analysis_log_file << "DIFF "
                        << std::to_string(m_onset_features[0] - m_first_noise -
                                          (1024 / 44100))
                        << std::endl;
  ...
  double temp_offset;
  m_analysis_log_file << "likely KICKS mean " << std::to_string(kick_mean)
                      << " stddev " << std::to_string(kick_sd) << std::endl;
  for (auto kick : likely_kicks) {
    if (kick > init_beat) {
      temp_offset =
          round((fmod((kick - init_beat), beat_interval) / beat_interval) *
                resolution);
      if (temp_offset == resolution) {
        temp_offset = 0; // rounding has casued us to make up a whole beat
      }

      m_analysis_log_file << std::to_string(kick) << " "
                          << std::to_string(temp_offset) << std::endl;
      likely_kick_beat_offsets.push_back(temp_offset);
      offset_count.at(temp_offset)++;
    }
  }

  std::for_each(offset_count.begin(), offset_count.end(), [this](int &n) {
    m_analysis_log_file << std::to_string(n) << std::endl;
  });
  m_analysis_log_file << "Initial beat is " << std::to_string(init_beat)
                      << std::endl;
  ...
  if (silence_to_first_onset > beat_interval) {
    m_analysis_log_file << "Using non-silence as starting point: "
                        << std::to_string(silence_to_first_onset) << std::endl;
    initial_onset_offset_time =
        fmod((m_first_noise - init_beat), beat_interval);
  } else {
    m_analysis_log_file << "Using first onset as starting point" << std::endl;
    initial_onset_offset_time =
        fmod((m_onset_features[0] - init_beat), beat_interval);
  }

  int initial_onset_offset_beat =
      round(initial_onset_offset_time / beat_interval * resolution);

  m_analysis_log_file << "First onset beat offset: "
                      << std::to_string(initial_onset_offset_beat) << std::endl;
  ...
  bool shift_good_for_kicks =
      offset_count[abs_initial_onset_offset_beat] > kick_per_beat_mean;

  if (!shift_good_for_kicks) {
    m_analysis_log_file << "Shift of : "
                        << std::to_string(initial_onset_offset_beat)
                        << " Not good for kicks, output: "
                        << std::to_string(
                               offset_count[abs_initial_onset_offset_beat])
                        << " mean: " << std::to_string(kick_per_beat_mean)
                        << std::endl;
    return 1;
  }
  ...
  m_analysis_log_file << "Initial beat shifted is " << std::to_string(init_beat)
                      << std::endl;
...
int analyzer::create_beat_grid(double &tempo,
                               double &first_beat) { // todo move to own class
...
std::vector<double>
analyzer::get_timestamps(Vamp::Plugin::FeatureList features) {
  std::vector<double> output;
  for (auto feature : features) {
    output.push_back(feature.timestamp.sec +
                     (double(feature.timestamp.nsec) / 1000000000));
  }
  return output;
}

double analyzer::get_distance(std::vector<double> &v1,
...
void analyzer::shift(std::vector<double> &v1, double shift_val) {
  std::for_each(v1.begin(), v1.end(),
                [shift_val](double &n) { n += shift_val; });
}

void analyzer::minimise_distance(std::vector<double> &move,
                                 std::vector<double> &nomove) {
...
void analyzer::get_mean_stddev(std::vector<double> input, double &mean,
                               double &stddev) {
  double cntr = 0;
...
void analyzer::get_mean_stddev(std::vector<int> input, double &mean,
                               double &stddev) {
  double cntr = 0;
  for (auto it = input.begin(); it != input.end(); ++it) {
    cntr += *it;
