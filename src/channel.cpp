...
int channel::read(float *data_ptr, int num_samples) {
  if (m_track) {
    ...
    if (read_samples != num_samples) {
      ...
      if (0) {
        m_bp.process_samples(data_ptr,
                             num_samples); // for filter testing purposes
      }
...
state_t channel::get_state() { return m_state; }

int channel::apply_action(action_t action) {
  if (action.control == LPF) {
    std::cout << "setting lpf gain to " << std::to_string(action.value)
              << " at time " << std::to_string(m_time) << std::endl;
    m_lpf.set_coefs(44100, kStartupLoFreq, kQKill, action.value);
  } else if (action.control == VOL) {
    std::cout << "setting volume to " << std::to_string(action.value)
              << " at time " << std::to_string(m_time) << std::endl;
    ...
    std::cout << "changing tempo at time " << std::to_string(m_time)
              << std::endl;
...
void channel::load(track *track) { m_track = track; }