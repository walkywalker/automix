...
void recorder::interleaved_to_planar(float* in_samples, float** out_samples, int length) {
...
void recorder::connect(mixer* input) {
	m_source = input;
}

int recorder::create_output() {
	avformat_alloc_output_context2(&m_format_ctx, nullptr, nullptr, m_path.c_str());
	if (!m_format_ctx) {
    ...
    std::cout << "encoder frame size" << std::to_string(m_codec_ctx->frame_size) << std::endl;
...
int recorder::get_frame_data_size() {
	return m_codec_ctx->frame_size*2;
}
...
int recorder::run() {
	...
	while (m_source -> read(samples, frame_data_length) == frame_data_length) {
	    ...
	    if (err < 0) {
	        std::cout << "Could not send packet for encoding: " << av_err2str(err) << std::endl;
	        std::cout << "Frame size: " << std::to_string(enc_frame->nb_samples) << std::endl;
	        return 1;
	    }
