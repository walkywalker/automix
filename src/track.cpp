...
void track::planar_to_interleaved(float** input_samples, float* output_samples, int length) {
...
int track::read(float *data_ptr, int num_samples) {
	...
	if (m_ring_buffer.read(data_ptr, num_samples) > 0) {
		...
		std::cout << "failed to read from track output buffer " << m_path << std::endl;
		return 0;
	}
}
...
int track::open_audio_source() {
	...
	if(error < 0) {
    	std::cout << "Error opening file " << m_path << " " << av_err2str(error) << std::endl;
    	return 1;
	}
	...
	m_audio_stream_index = av_find_best_stream(m_format_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, &m_codec, 0);
	...
	std::cout << m_path << ": Using codec " << m_codec->name << ", stream " << std::to_string(m_audio_stream_index) << std::endl;

	if (avcodec_parameters_to_context(m_codec_ctx, m_format_ctx->streams[m_audio_stream_index]->codecpar) < 0) {
	...
	std::cout << "	m_codec_ctx sample rate: " << std::to_string(m_codec_ctx->sample_rate) << std::endl;
	std::cout << "	m_codec_ctx channel_layout: " << std::to_string(m_codec_ctx->channel_layout) << std::endl;
	std::cout << "	m_codec_ctx channels: " << std::to_string(m_codec_ctx->channels) << std::endl;
	std::cout << "	m_codec_ctx bit_rate: " << std::to_string(m_codec_ctx->bit_rate) << std::endl;
	...
	while (m_ring_buffer.get_space() > frame_data_length) {
		...
		if (dec_pkt.stream_index == m_audio_stream_index) {
			...
			while (avcodec_receive_frame(m_codec_ctx, dec_frame) >= 0) {
				...
				if (m_ring_buffer.write(dec_samples, frame_data_length) != frame_data_length) {
